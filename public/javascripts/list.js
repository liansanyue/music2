 ...
 $(document).ready(function() { 
  getformusic(url,$("#tags .huayu ul"),"华语",1);
  getformusic(url,$("#tags .hanyu ul"),"日韩",1);
  getformusic(url,$("#tags .oumei ul"),"欧美",1);

  function getformusic(url,elem,language,page){
...
var audio=parent.document.querySelector("audio");
 ...
 $("i.play").click(function(){
    var lis= $(this).parent().next().children();
            ...
            $(showelem).css("display","block");
              for(var i= 0,l=result.length;i<l;i++){
                  if (this.currentTime /*当前播放的时间*/ > result[i][0]) {
                    if( $("#lrc p:eq("+i+")",window.parent.document).html().trim().length>1){
